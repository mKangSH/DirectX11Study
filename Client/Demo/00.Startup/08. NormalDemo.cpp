...
void NormalDemo::Init()
{
	...
	_shader = std::make_shared<Shader>(L"07. Normal.fx");
	...
	_geometry = std::make_shared<Geometry<VertexTextureNormalData>>();
	GeometryHelper::CreateSphere(_geometry);

	// height map data : 3 channels
	/*{
		std::vector<VertexTextureNormalData>& vertices = const_cast<std::vector<VertexTextureNormalData>&>(_geometry->GetVertices());

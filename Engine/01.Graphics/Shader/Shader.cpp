...
Shader::Shader(wstring file) : _file(L"..\\..\\Shaders\\" + file)
{
	...
	{
		DEVICECONTEXT->RSGetState(_initialStateBlock->RSRasterizerState.GetAddressOf());
		DEVICECONTEXT->OMGetBlendState(_initialStateBlock->OMBlendState.GetAddressOf(), _initialStateBlock->OMBlendFactor, &_initialStateBlock->OMSampleMask);
		DEVICECONTEXT->OMGetDepthStencilState(_initialStateBlock->OMDepthStencilState.GetAddressOf(), &_initialStateBlock->OMStencilRef);
	}
